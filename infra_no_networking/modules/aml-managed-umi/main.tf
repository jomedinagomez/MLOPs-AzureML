...
module "container_registry" {
  ...
  public_network_access_enabled = local.enable_private_networking ? false : true
  default_network_action        = local.enable_private_networking ? "Deny" : "Allow"
...
module "storage_account_default" {
  ...
  network_access_default   = local.enable_private_networking ? "Deny" : "Allow"
  network_trusted_services_bypass = [
    "AzureServices"
  ]
  resource_access = local.enable_private_networking ? [
    {
...
module "keyvault_aml" {
  ...
  firewall_default_action       = local.enable_private_networking ? "Deny" : "Allow"
...
resource "azapi_resource" "aml_workspace" {
  ...
  body = {
    ...
    properties = {
      ...
      managedNetworkKind = local.enable_private_networking ? "V1" : null
      ...
      publicNetworkAccess = local.enable_private_networking ? "disabled" : "enabled"

      # Configure the AML workspace to use the managed virtual network model when private networking is enabled
      managedNetwork = local.enable_private_networking ? {
      ...
      provisionNetworkNow = local.enable_private_networking
    }
...
module "private_endpoint_st_default_blob" {
  ...
  private_dns_zone_ids = compact([local.dns_zone_blob_id])
}
...
module "private_endpoint_st_default_file" {
  ...
  subnet_id            = var.subnet_id
  private_dns_zone_ids = compact([local.dns_zone_file_id])
}
...
module "private_endpoint_st_default_table" {
  ...
  subnet_id            = var.subnet_id
  private_dns_zone_ids = compact([local.dns_zone_table_id])
}
...
module "private_endpoint_st_default_queue" {
  ...
  subnet_id            = var.subnet_id
  private_dns_zone_ids = compact([local.dns_zone_queue_id])
}
...
module "private_endpoint_kv" {
  ...
  subnet_id            = var.subnet_id
  private_dns_zone_ids = compact([local.dns_zone_keyvault_id])
}
...
module "private_endpoint_container_registry" {
  ...
  subnet_id            = var.subnet_id
  private_dns_zone_ids = compact([local.dns_zone_acr_id])
}
...
resource "azapi_resource" "compute_cluster_uami" {
  ...
  enableNodePublicIp          = local.enable_private_networking ? false : true
  isolatedNetwork             = false
  osType                      = "Linux"
  remoteLoginPortPublicAccess = local.enable_private_networking ? "Disabled" : "Enabled"
        scaleSettings = {
          maxNodeCount                = 4
          minNodeCount                = 2
...
resource "azapi_resource" "compute_instance_uami" {
  ...
  enableNodePublicIp = local.enable_private_networking ? false : true
        ...
        sshSettings = {
          sshPublicAccess = local.enable_private_networking ? "Disabled" : "Enabled"
        }
        enableSSO                        = false
