...
resource "azurerm_role_assignment" "sp_dev_vnet_contributor" {
  count                = local.enable_private_networking ? 1 : 0
  scope                = azurerm_resource_group.dev_vnet_rg[0].id
...
resource "azurerm_role_assignment" "sp_prod_vnet_contributor" {
  count                = local.enable_private_networking ? 1 : 0
  scope                = azurerm_resource_group.prod_vnet_rg[0].id
  role_definition_name = "Contributor"
  principal_id         = azuread_service_principal.deployment_sp.object_id
...
resource "azurerm_virtual_network" "dev_vnet" {
  ...
  location            = var.location
  resource_group_name = azurerm_resource_group.dev_vnet_rg[0].name
...
module "dev_managed_umi" {
  ...
  resource_group_name                = azurerm_resource_group.dev_workspace_rg.name
  subnet_id                          = local.enable_private_networking ? azurerm_subnet.dev_pe[0].id : null
  ...
  resource_group_name_dns = local.enable_private_networking ? azurerm_resource_group.shared_dns_rg[0].name : null


  # Use shared AML DNS zones (per-env AML zones disabled in dev_vnet)
  dns_zone_aml_api_id       = local.enable_private_networking ? azurerm_private_dns_zone.shared_aml_api[0].id : null
  dns_zone_aml_notebooks_id = local.enable_private_networking ? azurerm_private_dns_zone.shared_aml_notebooks[0].id : null
  dns_zone_aml_instances_id = local.enable_private_networking ? azurerm_private_dns_zone.shared_aml_instances[0].id : null
...
resource "azurerm_virtual_network" "prod_vnet" {
  ...
  location            = var.location
  resource_group_name = azurerm_resource_group.prod_vnet_rg[0].name
  dns_servers         = var.dns_servers
...
resource "azurerm_subnet" "prod_bastion" {
  ...
  resource_group_name  = azurerm_resource_group.prod_vnet_rg[0].name
  virtual_network_name = azurerm_virtual_network.prod_vnet[0].name
...
resource "azurerm_network_security_group" "prod_vm_nsg" {
  ...
  security_rule {
    ...
    source_address_prefix      = azurerm_subnet.prod_bastion[0].address_prefixes[0]
...
resource "azurerm_subnet_network_security_group_association" "prod_vm_nsg_assoc" {
  count                     = local.enable_private_networking ? 1 : 0
  subnet_id                 = azurerm_subnet.prod_vm[0].id
  network_security_group_id = azurerm_network_security_group.prod_vm_nsg[0].id
}
...
resource "azurerm_bastion_host" "prod" {
  ...
  ip_configuration {
    name                 = "configuration"
    subnet_id            = azurerm_subnet.prod_bastion[0].id
...
resource "azurerm_network_interface" "prod_vm_nic" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.prod_vm[0].id
  }
...
resource "azurerm_windows_virtual_machine" "jumpbox" {
  ...
  network_interface_ids = [
    azurerm_network_interface.prod_vm_nic[0].id
  ]
...
resource "azurerm_virtual_network_peering" "dev_to_prod" {
  ...
  resource_group_name       = azurerm_resource_group.dev_vnet_rg[0].name
  virtual_network_name      = azurerm_virtual_network.dev_vnet[0].name
  remote_virtual_network_id = azurerm_virtual_network.prod_vnet[0].id
...
resource "azurerm_virtual_network_peering" "prod_to_dev" {
  ...
  resource_group_name       = azurerm_resource_group.prod_vnet_rg[0].name
  virtual_network_name      = azurerm_virtual_network.prod_vnet[0].name
  remote_virtual_network_id = azurerm_virtual_network.dev_vnet[0].id
...
module "prod_managed_umi" {
  ...
  resource_prefixes                  = local.resource_prefixes
  resource_group_name                = azurerm_resource_group.prod_workspace_rg.name
  subnet_id                          = local.enable_private_networking ? azurerm_subnet.prod_pe[0].id : null
...
resource "azurerm_private_dns_a_record" "shared_aml_instances_wildcard" {
  ...
  name                = "*.${var.location}"
  zone_name           = azurerm_private_dns_zone.shared_aml_instances[0].name
  resource_group_name = azurerm_private_dns_zone.shared_aml_instances[0].resource_group_name
...
resource "azurerm_private_dns_zone_virtual_network_link" "shared_dev_blob" {
  ...
  resource_group_name   = azurerm_resource_group.shared_dns_rg[0].name
  private_dns_zone_name = azurerm_private_dns_zone.shared_blob[0].name
  virtual_network_id    = azurerm_virtual_network.dev_vnet[0].id
...
resource "azurerm_private_dns_zone_virtual_network_link" "shared_dev_file" {
  ...
  resource_group_name   = azurerm_resource_group.shared_dns_rg[0].name
  private_dns_zone_name = azurerm_private_dns_zone.shared_file[0].name
  virtual_network_id    = azurerm_virtual_network.dev_vnet[0].id
  registration_enabled  = false
...
resource "azurerm_private_dns_zone_virtual_network_link" "shared_dev_queue" {
  ...
  resource_group_name   = azurerm_resource_group.shared_dns_rg[0].name
  private_dns_zone_name = azurerm_private_dns_zone.shared_queue[0].name
  virtual_network_id    = azurerm_virtual_network.dev_vnet[0].id
  registration_enabled  = false
...
resource "azurerm_private_dns_zone_virtual_network_link" "shared_dev_table" {
  ...
  resource_group_name   = azurerm_resource_group.shared_dns_rg[0].name
  private_dns_zone_name = azurerm_private_dns_zone.shared_table[0].name
  virtual_network_id    = azurerm_virtual_network.dev_vnet[0].id
  registration_enabled  = false
...
resource "azurerm_private_dns_zone_virtual_network_link" "shared_dev_vault" {
  ...
  resource_group_name   = azurerm_resource_group.shared_dns_rg[0].name
  private_dns_zone_name = azurerm_private_dns_zone.shared_vault[0].name
  virtual_network_id    = azurerm_virtual_network.dev_vnet[0].id
  registration_enabled  = false
...
resource "azurerm_private_dns_zone_virtual_network_link" "shared_dev_acr" {
  ...
  name                  = "dev-shared-acr"
  resource_group_name   = azurerm_resource_group.shared_dns_rg[0].name
  private_dns_zone_name = azurerm_private_dns_zone.shared_acr[0].name
  virtual_network_id    = azurerm_virtual_network.dev_vnet[0].id
  registration_enabled  = false
...
resource "azurerm_private_dns_zone_virtual_network_link" "shared_prod_blob" {
  ...
  resource_group_name   = azurerm_resource_group.shared_dns_rg[0].name
  private_dns_zone_name = azurerm_private_dns_zone.shared_blob[0].name
  virtual_network_id    = azurerm_virtual_network.prod_vnet[0].id
  registration_enabled  = false
...
resource "azurerm_private_dns_zone_virtual_network_link" "shared_dev_api" {
  ...
  resource_group_name   = azurerm_resource_group.shared_dns_rg[0].name
  private_dns_zone_name = azurerm_private_dns_zone.shared_aml_api[0].name
  virtual_network_id    = azurerm_virtual_network.dev_vnet[0].id
  registration_enabled  = false
...
resource "azurerm_private_dns_zone_virtual_network_link" "shared_dev_notebooks" {
  ...
  resource_group_name   = azurerm_resource_group.shared_dns_rg[0].name
  private_dns_zone_name = azurerm_private_dns_zone.shared_aml_notebooks[0].name
  virtual_network_id    = azurerm_virtual_network.dev_vnet[0].id
  registration_enabled  = false
...
resource "azurerm_private_dns_zone_virtual_network_link" "shared_dev_instances" {
  ...
  resource_group_name   = azurerm_resource_group.shared_dns_rg[0].name
  private_dns_zone_name = azurerm_private_dns_zone.shared_aml_instances[0].name
  virtual_network_id    = azurerm_virtual_network.dev_vnet[0].id
  registration_enabled  = false
...
output "shared_aml_dns_zone_ids" {
  value = local.enable_private_networking ? {
    api       = azurerm_private_dns_zone.shared_aml_api[0].id
    notebooks = azurerm_private_dns_zone.shared_aml_notebooks[0].id
    instances = azurerm_private_dns_zone.shared_aml_instances[0].id
  } : tomap({})
}
...
resource "azurerm_role_assignment" "user_dev_rg_reader" {
  ...
  depends_on           = [module.dev_managed_umi]
}
...
resource "azurerm_role_assignment" "user_dev_registry_user" {
  ...
  role_definition_name = "AzureML Registry User"
  principal_id         = data.azurerm_client_config.current.object_id
  depends_on           = [module.dev_registry]
}
...
resource "azurerm_role_assignment" "user_prod_rg_reader" {
  ...
  role_definition_name = "Reader"
  principal_id         = data.azurerm_client_config.current.object_id
  depends_on           = [module.prod_managed_umi]
}
...
resource "azurerm_role_assignment" "user_prod_storage_file_privileged_contributor" {
  ...
  role_definition_name = "Storage File Data Privileged Contributor"
  principal_id         = data.azurerm_client_config.current.object_id
  depends_on           = [module.prod_registry]
}
